...
export default function Dashboard() {
  ...
  const [selectedBuildingPreview, setSelectedBuildingPreview] = useState<string | null>(null);
        ...
        {selectedBuildingPreview && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setSelectedBuildingPreview(null)}
              className="absolute inset-0 bg-gray-900/60 backdrop-blur-xl"
            />
            <motion.div
              initial={{ opacity: 0, scale: 0.9, y: 40 }}
            ...
            >
              <div className="p-10 border-b border-gray-100 flex items-center justify-between bg-gray-900 text-white">
                <div>
                  <h2 className="text-3xl font-black uppercase tracking-tight leading-none">{selectedBuildingPreview}</h2>
                  <p className="text-gray-400 text-[10px] font-black mt-2 tracking-widest uppercase">Building Consumption Breakdown</p>
                </div>
                <button 
                  onClick={() => setSelectedBuildingPreview(null)}
                  className="p-3 hover:bg-white/10 rounded-2xl transition-all"
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-8">
                {(() => {
                  const bTransactions = dateFilteredTransactions
                    .filter(t => t.building?.trim() === selectedBuildingPreview.trim() && t.type === 'out')
                    .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());

                  const materialAggregation: Record<string, { quantity: number, value: number, unit: string }> = {};
                  bTransactions.forEach(t => {
                    const material = materials.find(m => m.id === t.materialId);
                    const name = t.materialName || material?.name || 'Unknown Item';
                    const unitPrice = Number(t.unitPrice) || Number(material?.unitPrice) || 0;
                    const val = (Number(t.quantity) || 0) * unitPrice;
                    
                    if (!materialAggregation[name]) {
                      materialAggregation[name] = { quantity: 0, value: 0, unit: material?.unit || 'pcs' };
                    }
                    materialAggregation[name].quantity += Number(t.quantity) || 0;
                    materialAggregation[name].value += val;
                  });

                  const totalBuildingValue = Object.values(materialAggregation).reduce((sum, item) => sum + item.value, 0);

                  return (
                    <>
                      <div className="space-y-4">
                        <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Material Aggregation</h4>
                        <div className="grid grid-cols-1 gap-4">
                          {Object.entries(materialAggregation)
                            .sort((a, b) => b[1].value - a[1].value)
                            .map(([name, data]) => {
                              const percentage = totalBuildingValue > 0 ? (data.value / totalBuildingValue) * 100 : 0;
                              return (
                                <div key={name} className="space-y-3 p-5 bg-gray-50 rounded-2xl border border-gray-100 group hover:bg-blue-50 transition-colors">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                      <p className="text-lg font-black text-gray-900 tracking-tighter">₹{data.value.toLocaleString()}</p>
                                      <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-0.5">{percentage.toFixed(1)}%</p>
                                    </div>
                                  </div>
                          ...
                          {Object.keys(materialAggregation).length === 0 && (
                            <div className="text-center py-12 bg-gray-50 rounded-[2rem] border border-dashed border-gray-200">
                              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">No consumption records found</p>
                            </div>
                          )}
                        </div>
                      </div>

                      {bTransactions.length > 0 && (
                        <div className="space-y-4">
                          <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">Recent Issuances</h4>
                          <div className="space-y-3">
                            {bTransactions.slice(0, 10).map((t, idx) => (
                              <div key={t.id} className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100 shadow-sm">
                                <div className="flex items-center gap-3">
                                  <div className="w-2 h-2 rounded-full bg-blue-600" />
                                  <div>
                                    <p className="text-xs font-bold text-gray-900">{t.materialName}</p>
                                ...
                                <div className="text-right">
                                  <p className="text-xs font-black text-gray-900">{t.quantity}</p>
                                </div>
                              </div>
                            ))}
                          </div>
              ...
              <div className="p-10 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                <span className="text-xs font-black text-gray-500 uppercase tracking-[0.2em]">Total Building Valuation</span>
                <span className="text-4xl font-black text-gray-900 tracking-tighter">
                  ₹{(() => {
                    const bTransactions = dateFilteredTransactions.filter(t => t.building?.trim() === selectedBuildingPreview.trim() && t.type === 'out');
                    return bTransactions.reduce((acc, t) => {
                      const material = materials.find(m => m.id === t.materialId);
                      const unitPrice = Number(t.unitPrice) || Number(material?.unitPrice) || 0;
                      return acc + ((Number(t.quantity) || 0) * unitPrice);
                    }, 0).toLocaleString();
              ...
              {buildings.map((building) => {
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => setSelectedBuildingPreview(building.name)}
                    className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-xl shadow-gray-200/40 ring-1 ring-black/5 transition-all group"
                  >
